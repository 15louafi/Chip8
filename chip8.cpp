...
void Chip8::OP_8XY4()
{
    uint8_t x = (opcode & 0x0F00u) >> 8u;
    uint8_t y = (opcode & 0x00F0u) >> 4u;
    uint16_t result = register[x]+register[y];
    if(result > 0x00FFu) {
        register[0xF] = 1;
    }
    else {
...
{
    uint8_t x = (opcode & 0x0F00u) >> 8u;
    register[0xF] = (registers[Vx] & 0x80u) >> 7u;
    register[x] <<= 1;
}
...
{
    uint8_t value = (opcode & 0x0FFFu);
    programCounter = register[0] + value;
}
